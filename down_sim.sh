...
OUTPUT_DIR=$SIM_OUTPUT_DIR/$DATE
if [[ ! -d $OUTPUT_DIR ]]; then
    mkdir -p $OUTPUT_DIR
fi

# iterate over days
for DAY in $(seq ${SIM_FIRST_DAY} ${SIM_LAST_DAY}); do
	...
	OUTFILE_NAME="${CURRDATE}_d-MERCATOR--${FILE_ACRONYM}--${REGION_ACRONYM}-b${DATE}_sm-fv01.nc"
	...
	if [[ ! -e ${OUTPUT_DIR}/${OUTFILE_NAME} ]]; then
	    ...
	    ncrename -d latitude,lat -d longitude,lon -v latitude,lat -v longitude,lon ${OUTPUT_DIR}/${OUTFILE_NAME}
